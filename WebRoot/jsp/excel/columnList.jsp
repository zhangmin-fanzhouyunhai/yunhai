...
<BODY>
	<FORM id="sheetList" name="sheetList"
						...
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR class=manageHead>
								<TD>当前位置：<A class=style2
									...
									target=main>excel文件清单</A> &gt;<A class=style2
									href="${pageContext.request.contextPath}/sheetListServlet?owner=admin&fileName=${fileName}"
									target=main>${fileName}</A> &gt; 数据列清单
								</TD>
																...
																<c:forEach items="${excelColumnValueTypeList }"
																	...
																	<c:if
																		...
																		<option title="dataTypeValue"
																			value="${dataTypeValue.dataTypeCode }"
																			selected="selected">${dataTypeValue.columnValueTypeName }</option>
																	</c:if>
																	<c:if
																		test="${dataTypeValue.dataTypeCode != column.dataTypeCode}">
																		<option title="dataTypeValue"
