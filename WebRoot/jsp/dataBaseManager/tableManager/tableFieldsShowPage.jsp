...
<body>
	...
	<table border="1" align="center" class="table table-striped" width="90%">
		<tr align="center">
		...
		第${pageBean.currentPage}页/共${pageBean.totalPageCount}页
			 ...
			 --%>
		<tr>
			<td align="left" colspan="3">显示记录数：<select name="pageSize"
		...
		[上一页]
				...
				</c:if> 第 <select name="currentPage" id="currentPage"
			...
			</select> 页 <c:if test="${ pageBean.currentPage==pageBean.totalPageCount}">
				...
				</c:if> <c:if test="${ pageBean.currentPage==pageBean.totalPageCount-1}">
